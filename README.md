# 🎮 Games by it-stud.io

Eine Sammlung von Browser-Spielen im it-stud.io Design — keine Installation nötig, einfach öffnen und spielen!

🌐 **Live spielen:** https://itstud-io.github.io/games/

---

## Spiele

### 🧱 Tetris

Der Arcade-Klassiker im modernen Dark-Mode-Design.

**Features:**
- Klassisches Tetris-Gameplay
- Vorschau des nächsten Steins
- Score, Level & Linien-Tracking
- Tastatursteuerung (← → ↓ für Bewegung, ↑ für Rotation)
- Responsive Design

▶️ **[Jetzt spielen](https://itstud-io.github.io/games/tetris.html)**

---

### 🏰 Dungeon Explorer

Ein Action-Adventure durch mysteriöse Dungeon-Räume.

**Features:**
- Prozedural generierte Räume
- Erkunde dunkle Dungeons mit Fackel-Beleuchtung
- Sammle Schätze und weiche Fallen aus
- Finde den Ausgang in jedem Level
- Atmosphärisches Dark-Fantasy-Design
- Tastatursteuerung (WASD oder Pfeiltasten)

▶️ **[Jetzt spielen](https://itstud-io.github.io/games/dungeon-explorer.html)**

---

## Technologie

- **Pure HTML/CSS/JavaScript** — keine externen Abhängigkeiten
- **Standalone** — jedes Spiel ist eine einzelne HTML-Datei
- **Offline-fähig** — einmal geladen, überall spielbar
- **it-stud.io Design** — einheitliches Dark-Theme mit Akzentfarben

---

## Lokal spielen

```bash
# Repository klonen
git clone git@github.com:ITSTUD-IO/games.git

# Spiel im Browser öffnen
open games/tetris.html
# oder
open games/dungeon-explorer.html
```

---

## Lizenz

[MIT License](LICENSE) — Feel free to play, modify & share!

---

<p align="center">
  <i>Gebaut mit ⚡ von <a href="https://github.com/simon-itstudio">Simon</a> @ <a href="https://it-stud.io">it-stud.io</a></i>
</p>
